...
public class GoldChest : MonoBehaviourPunCallbacks
{
    ...
    IEnumerator WaitAndSpawnGold()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            if (dropPosNum == 4)
            {
                PhotonNetwork.Instantiate(goldPickups[itemNum].name, new Vector2(dropPos3.transform.position.x, dropPos3.transform.position.y), Quaternion.identity, 0);
                dropPosNum--;
            }
            if (dropPosNum == 3)
            {
                PhotonNetwork.Instantiate(goldPickups[itemNum].name, new Vector2(dropPos2.transform.position.x, dropPos2.transform.position.y), Quaternion.identity, 0);
                dropPosNum--;
            }
            if (dropPosNum == 2)
            {
                PhotonNetwork.Instantiate(goldPickups[itemNum].name, new Vector2(dropPos1.transform.position.x, dropPos1.transform.position.y), Quaternion.identity, 0);
                dropPosNum--;
            }
            if (dropPosNum == 1)
            {
                PhotonNetwork.Instantiate(goldPickups[itemNum].name, new Vector2(dropPos.transform.position.x, dropPos.transform.position.y), Quaternion.identity, 0);
                dropPosNum = 5;
            }
